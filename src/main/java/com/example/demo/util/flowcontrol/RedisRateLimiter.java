...
public class RedisRateLimiter {
    ...
    public boolean tryAcquire(String key, int permits, long timeout, TimeUnit unit) throws IllegalArgumentException {
        ...
        try {
            locked = lock.lock(key + LOCK_KEY_SUFFIX, String.valueOf(Thread.currentThread().getId()), 60, 2, TimeUnit.SECONDS);
            if (locked) {
            ...
            if (locked) {
                lock.unLock(key + LOCK_KEY_SUFFIX, String.valueOf(Thread.currentThread().getId()));
            }
        }
        if (millisToWait > 0) {
            try {
    ...
    private long reserve(String key, int permits) throws IllegalArgumentException {
        checkPermits(permits);
        try {
            lock.lock(key + LOCK_KEY_SUFFIX, String.valueOf(Thread.currentThread().getId()), 60, 2, TimeUnit.SECONDS);
            long nowMillis = getNowMillis();
            RedisPermits permit = getPermits(key, nowMillis);

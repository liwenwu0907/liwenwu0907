...
function con_ContractFileIn(_fn) {
    ...
    self.searchContract = function () {
        ...
        layui.use(['layer'], function () {
            ...
            postDataToServer("/applyModel/getJyhtSpf",{hth: contractNo,page:10,size:0},function(data){
                if (data && data.head.returncode == "0000") {
                    ...
                    data.htxxList.forEach(function (item) {
                        var tmplData = {
                            htxx: item,
                            hth: item.fcjyXjspfMmht.htbh,
                            ywh: item.fcjyXjspfMmht.htid,
                            zl: item.fcjyXjspfMmht.htzl,
                            qlrmc: item.fcjyXjspfMmhtZtList.map(function (q) {
                                return q.ztxm;
                            }).join("、"),
                            qlrzjzl: item.fcjyXjspfMmhtZtList.map(function (q) {
                                return q.ztzjlb;
                            }).join("、"),
                            qlrzjh: item.fcjyXjspfMmhtZtList.map(function (q) {
                                return q.ztzjhm;
                            }).join("、"),
                        }
...
function importContract (data) {
    ...
    $("#mmhth").val(data.hth)
    //权利人信息--数组
    var qlrxxList = data.htxx.fcjyXjspfMmhtZtList;
    ...
    if (qlrxxList.length > 0){
        ...
        qlrxxList.forEach(function (item) {
            ...
            //item.qlrlb == 2 义务人
            if ((item.ztlb == 1 || item.ztlb == "1") && $("#personTwo").length > 0) {
                if (1 == ywrlen) {
...
function forEachInsert(item, type) {
    if (1 == type) {
        ...
        //权利人名称qlrmc
        $("#obligeers").find("input[name=qlrmc][type=text]").val(item.ztxm);
        //证件种类qlrzjzl
        $("#obligeers").find("select[name='qlrzjzl']").find("option[value = '" + item.ztzjlb + "']").attr("selected", "selected");
        //权利人证件号qlrzjh
        $("#obligeers").find("input[name=qlrzjh][type=text]").val(item.ztzjhm);
        /*//权利人邮编qlryb
        $("#personOne").find("input[name=qlryb][type=text]").val(item.YB);
        //权利人地址DZ qlrtxdz
        $("#personOne").find("input[name=qlrtxdz][type=text]").val(item.DZ);
        //权利人电话fddbrhfzrdh
        $("#personOne").find("input[name=qlrlxdh][type=text]").val(item.DH);*/
    }
    if (2 == type) {
        //权利人类型
        ...
        //证件种类qlrzjzl
        $("#obligors").find("select[name='qlrzjzl']").find("option[value = '" + item.ztzjlb + "']").attr("selected", "selected");
        //权利人证件号qlrzjh
        $("#obligors").find("input[name=qlrzjh][type=text]").val(item.ztzjhm);
        //权利人邮编qlryb
